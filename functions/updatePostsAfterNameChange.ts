...
async function updatePostsInBatches(posts, newFullName) {
    ...
    for (let i = 0; i < posts.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            console.error(`[updatePostsAfterNameChange] Error updating batch starting at index ${i}:`, error);
...
Deno.serve(async (req) => {
    try {
        ...
        if (!newFullName) {
            return new Response(JSON.stringify({ error: 'New full name not provided' }), { 
                status: 400, 
                headers: { 'Content-Type': 'application/json' } 
            });
        }
        ...
        const updatedCount = await updatePostsInBatches(userPosts, newFullName);
        ...
        console.error("[updatePostsAfterNameChange] Function Error:", error);
        return new Response(JSON.stringify({ 
            error: 'Failed to synchronize name to posts.', 
